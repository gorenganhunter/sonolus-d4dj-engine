...
export class SliderFlickParticle extends SpawnableArchetype({ start: Number, startLane: Number, direction: Number }) {
    ...
    updateParallel() {
        if (time.now > this.spawnData.start + 0.05 * Math.abs(this.spawnData.direction)) this.despawn = true
        if ((this.i + 0.525) / 1.05 > Math.abs(this.spawnData.direction) * 2) this.despawn = true
